...
IF EXISTS (select * FROM sys.objects WHERE type = 'P' AND name = 'pr_LinkMasterCancerNotification')
DROP PROCEDURE [pr_LinkMasterCancerNotification]
...
/*=========================================================================================
   ...
   DESCRIPTION:     Master to manage the cancer notification import to DL

	USE:			EXEC pr_LinkMasterCancerNotification
    
	PARAMETERS:		 

    REVISIONS:		06/08/2021	DJF		Created 
					
-- ========================================================================================*/


CREATE PROCEDURE [dbo].[pr_LinkMasterCancerNotification]  
...
Set @EmailBodyMessage = '<B>Processing Cancer Notification (' + @EnvironmentName + ')</B><BR><BR>'

BEGIN TRY

	--STEP-1:  Move the data to staging ; select count(*) from tblLinkStagingCancerNotification
	Set @EmailBodyMessage = @EmailBodyMessage + 'Step 1 - Moving data from landing to staging<BR>'
	EXEC envido_dl.dbo.pr_LinkStagingCancerNotification


	-- Step-2: Retrofit the observation value as needed - Found in tblLinkStagingCancerNotification
	Set @EmailBodyMessage = @EmailBodyMessage + 'Step 2 - Not needed for cancer notification<BR>'
	-- ##### This is not needed for Cancer notification as an ObservationValue already comes across from the Cancer Notification system #####
