...
BEGIN
	...
	*/
    ...
    --IF  EXISTS (Select * From sys.objects Where object_id = OBJECT_ID(N'[dbo].[MatchingTemp]') AND type in (N'U'))
    --DROP TABLE [dbo].[MatchingTemp]  
    
    IF OBJECT_ID('tempdb..#MatchingTemp') IS NOT NULL DROP TABLE #MatchingTemp
    Create table #MatchingTemp (
            bID  int IDENTITY (1,1) NOT NULL,
            bSourceRecordID int  NULL,
            bFirstName nvarchar(50) NULL,
            bFnInit nchar (1) NULL,
    ...
    --IF EXISTS (Select * From sysobjects Where type  =  'U' AND name  =  'MatchingResults')
	...
	--IF EXISTS (Select * From sys.objects Where type = 'U' AND name = 'MatchingWork1') DROP Table [MatchingWork1]
    ...
    --IF EXISTS (Select * From sysobjects Where type  =  'U' AND name  =  'MatchingWork1')
    --    DROP TABLE MatchingWork1
    IF OBJECT_ID('tempdb..#MatchingWork1') IS NOT NULL DROP TABLE #MatchingWork1      
    ------------------------------------------------------------------------------------------------
    ...
    --IF EXISTS (Select * From sysobjects Where type  =  'U' AND name  =  'MatchingResults') DROP TABLE MatchingResults
    IF OBJECT_ID('tempdb..#MatchingResults') IS NOT NULL DROP TABLE #MatchingResults    
    
    Select  *
    Into    #MatchingResults
   ...
   ****************************************************************************/
	...
	Begin 
		...
		SELECT '#MatchingResults',count(*) from #MatchingResults
